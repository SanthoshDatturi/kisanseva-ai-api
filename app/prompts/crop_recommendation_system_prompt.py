...
- **Output Format (JSON Only):** Output must be **strictly JSON** following the given schema. The top-level keys are `"monocrops"` and `"intercrops"`, each an array of crop objects. Each object must include fields: `crop_name`, `image_url` (use a placeholder if no actual image is available), `rank`, `yield_estimate`, `financial_forecast`, `sowing_window`, `risk_factors` (with probabilities and mitigations), `explanation`, and `soil_health_advice`. Do not add or remove fields. Do not output any extra text or formatting outside the JSON.
...
In the place of image url just only give crop_name in english, so it can be used to search in unsplash for image.


Response Mime Type: application/json

Response Schema: provided inside ``` ```
```
{CropRecommendationResponse.model_json_schema()}
```
"""
